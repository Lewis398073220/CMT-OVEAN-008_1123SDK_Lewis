...
uint32_t cst820_read_checksum(void){
 ...
 bool ctp_hynitron_update(void)
 {
	...
	cst820_rst_init();
    if (cst820_enter_bootmode() == 0){			//进入boot模式
#include "capacitive_hynitron_cst820_update.h"
        if(sizeof(app_bin) > 10){
            uint16_t startAddr = app_bin[1];	//app_bin是capacitive_hynitron_cst820_update.h中的数组
            ...
            if(cst820_read_checksum() != checksum){			//读取IC里面的校验和，与固件的不一致时就进入升级流程
                cst820_update(startAddr, length, app_bin+6);	//开始执行升级流程
                cst820_read_checksum();						//读出新烧录进去的校验和
            }
        }
        return CTP_TRUE;
    }
...
#include "capacitive_hynitron_cst820_update.h"
        if(sizeof(app_bin) > 10){
            kal_uint16 startAddr = app_bin[1];//app_bin是capacitive_hynitron_cst820_update.h中的数组
			...
			//以上是获取升级固件的一些信息			
            if(cst820_read_checksum(startAddr, length) != checksum){//读取IC里面的校验和，与固件的不一致时就进入升级流程
                cst820_update(startAddr, length, app_bin+6);//开始执行升级流程
                cst820_read_checksum(startAddr, length);//读出新烧录进去的校验和
				//
            }
        }
        return CTP_TRUE;

...
const unsigned char AC107_Register_48khz_Config[][2]={
        ...
        {0x32, 0x00},  //FS = 32, 0x20-1=0x1f  Modify by lewis from 0x10 to 0x00 because the left and right channels are reversed
        ...
        {0x61, 0x07}, 
        {0x66, 0x03}, 
        {0x70, 0xAD}, //9.75dB Modify by lewis
        {0x71, 0xAD}, //9.75dB Modify by lewis
        {0x76, 0x01}, 
        {0x77, 0x02}, 
        {0x7F, 0x00},  /* ADC */
        {0xA0, 0x00}, 
...
{
#ifdef CMT_008_AC107_ADC
	ASSERT(cfg_hw_ac107_ldo_enable.pin != HAL_IOMUX_PIN_NUM, "[%s] !!!ac107 ldo pin is not defined", __func__);

	hal_gpio_pin_set_dir((enum HAL_GPIO_PIN_T)cfg_hw_ac107_ldo_enable.pin, HAL_GPIO_DIR_OUT, 0);
#endif

#ifdef CMT_008_LDO_1V8_ENABLE
	ASSERT(cfg_hw_ldo_1v8_enable.pin != HAL_IOMUX_PIN_NUM, "[%s] !!!ldo 1v8 pin is not defined", __func__);
...
void ac107_hw_open(void)
{
	...
	hal_gpio_pin_set_dir((enum HAL_GPIO_PIN_T)cfg_hw_ac107_ldo_enable.pin, HAL_GPIO_DIR_OUT, 1);
#endif
}

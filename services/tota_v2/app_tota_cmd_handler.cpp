...
void user_custom_app_tota_cmd_handle(uint8_t* ptrData, uint16_t dataLength)
{
	TOTA_LOG_DBG(2 ,"[%s] TOTAconnectionPath:[%d]",__func__, tota_get_connect_path());
	
	switch (tota_get_connect_path())
    {
        case APP_TOTA_VIA_SPP:
			//handle spp packet here
			app_tota_cmd_received(ptrData, dataLength);

			//if allow the app send packet via spp, need to handle app cmd either here
			//tota_status = app_tota_cmd_received(ptrData, dataLength);
			//if(tota_status != TOTA_NO_ERROR) custom_tota_ble_data_handle(ptrData, dataLength);
			break;
			
#ifdef BLE_TOTA_ENABLED
        case APP_TOTA_VIA_NOTIFICATION:
			//handle BLE packet here
            TOTA_LOG_DBG(2 ,"[%s] BLEconnectionIndex:[%d]",__func__, tota_ble_get_conidx());
			user_custom_tota_ble_data_handle(ptrData, dataLength);
			break;
#endif
